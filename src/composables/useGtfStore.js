...
export function useGtfStore() {
  ...
  function updateGlyphData({ field, value, action }) {
    ...
    if (field === 'char_repr') {
        const canAutoPopulate = value && value.length > 0 && !currentGlyph.unicode;
        if (canAutoPopulate) {
            try {
            ...
            } catch (e) { console.error("GTF Store: Error getting code point:", e); }
        }
        ...
        if (processedValue === null) { console.error("GTF Store: Glyph name cannot be empty."); return; }
        ...
        if (processedValue.length === 1) {
            ...
            if (!currentGlyph.unicode) {
                ...
                } catch (e) { console.error("GTF Store: Error getting code point for auto-unicode from name:", e); }
            }
        }
      ...
      currentGlyph.palette = value && typeof value === 'object' ? value : { entries: {} };

    } else if (field === 'bitmap') {
       if (Array.isArray(value)) { currentGlyph.bitmap = value; }
       else { console.error("GTF Store: Invalid value received for bitmap update."); }
       
    } else if (field) {
        console.warn(`GTF Store: Updating unhandled field '${field}' generically.`);
        currentGlyph[field] = value; 
    } else {
        console.error(`GTF Store: Attempted to update unknown field '${field}' on glyph.`);
    }
  }
  ...
  function addGlyphForChar(char) {
    ...
    let baseName = char.match(/[a-zA-Z0-9]/) ? char : 'Glyph';
    ...
    if (!newGlyph.unicode && newGlyph.char_repr) {
       ...
       } catch (e) { console.error("GTF Store: Error getting code point for new char:", e); }
    }
