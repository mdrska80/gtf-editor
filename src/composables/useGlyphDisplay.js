...
const allCommon =
  commonLowercase + commonUppercase + commonDigits + commonPunctuation;
...
export function useGlyphDisplay(reactiveGlyphs) {
  ...
  function toggleSidebarView() {
    ...
    console.log(
      'GlyphDisplay: Toggled sidebar view to',
      isSimplePreviewMode.value ? 'simple' : 'grouped'
    );
  }
  ...
  const sortedGlyphs = computed(() => {
    ...
    return [...glyphs].sort(
      (a, b) => parseUnicode(a.unicode) - parseUnicode(b.unicode)
    );
  });
  ...
  const groupedGlyphs = computed(() => {
    ...
    const languageCharsSet = new Set(
      Object.values(languageCharacterSets)
        .filter((set) => set !== allCommon)
        .join('')
        .replace(
          new RegExp(
            `[${allCommon.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}]`,
    ...
    for (const glyph of glyphsToSort) {
      ...
      else if (char && languageCharsSet.has(char))
